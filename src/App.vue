<template>
  <v-app>
    ...
    <v-dialog v-model="fileSaveDialog">
      <v-card>
        <v-card-title>Simpan Kuesioner</v-card-title>
        <v-card-text>
          <v-text-field v-model="fileName" label="Nama File"></v-text-field>
        </v-card-text>
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn @click="(fileSaveDialog = false)">Batal</v-btn>
            <v-btn @click="saveKuesioner">Simpan</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="fileExistDialog">
        <v-card>
            <v-card-title>
                File {{fileName}}.json telah ada, apakah anda ingin menimpanya?
            </v-card-title>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn @click="(fileExistDialog = false)">Batal</v-btn>
                <v-btn @click="saveNotExist">Simpan</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
    <v-snackbar
...
export default {
  ...
  methods: {
    ...
    saveKuesioner(){
        window.resolveLocalFileSystemURL(`${this.donwloadPath}/`, this.checkExisting, this.fail);
    },
    saveNotExist(){
        window.resolveLocalFileSystemURL(`${this.donwloadPath}/`, this.saveResolve, this.fail);
    },
    saveResolve(dir){
        dir.getFile(`${this.fileName}.json`, {create:true}, this.writeFile, this.fail);
    },
    writeFile(file){
    ...
    checkExisting(dir){
        dir.getFile(`${this.fileName}.json`, {create:false}, this.fileExist, this.saveNotExist, this.fail);
    },
    // eslint-disable-next-line no-unused-vars
    fileExist(e){
    ...
    readFile(e){
      ...
      window.resolveLocalFileSystemURL(e, this.resolveFile, this.failRead)
    },
    resolveFile(fs){
      const vm = this
      fs.file(function(res) {
        ...
        reader.onloadend = function(e) {
          let res = JSON.parse(e.target._result)
          vm.result = res
          vm.$store.commit("setKuesioner", res)
        //   vm.created = true
        };
    ...
    successChoose(e){
      ...
      window.FilePath.resolveNativePath(e, this.readFile, this.failResolvePath)
    },
    success(){
  ...
  data: () => ({
    deviceReady: false,
    file: '',
    fileName: 'Untitled Kuesioner',
    fileExistDialog: false,
    fileSaveDialog: false,
    err: '',
    // success: '',
    result: '',
    donwloadPath: 'file:///storage/emulated/0/download',
    logOb: null,
    created: false,
    saveSnack: false,
    fileError: false,

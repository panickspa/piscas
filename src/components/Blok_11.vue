<template>
    <v-container>
        <v-card>
            <v-card-title>
                <h3>{{ $store.state.kuesioner.blok_catatan.nama }}</h3>
            </v-card-title>
            <div class="d-flex flex-column mr-4 ml-4">
                <k-rincian 
                    v-for="r in $store.state.kuesioner.blok_catatan.rincian" 
...
import RincianVue from './kuesioner/Rincian.vue'
    export default {
        ...
        mounted(){
            this.blok = this.$store.state.kuesioner.blok_catatan
        },
        data(){
            return {
                err: null,

<template>
    <div style="height: 100%">
        <v-container>
            <v-card>
                <v-card-title>
                    <h3>{{ $store.state.kuesioner.blok_10.nama }}</h3>
                ...
                <div class="d-flex flex-column">
                    <v-list dense>
                        <v-list dense>
                            <v-list-item
                                v-for="(item, i) in $store.state.kuesioner.blok_10.isi"
                                :key="i+`-blok_10-list`"
                                @click="editRow(i)"
                            >
                                <v-list-item-content>
        ...
        <d-rincian-edit
            ...
            :rincian="$store.state.kuesioner.blok_10.rincian"
            :blok="'blok-4'"
...
import DialogRincianEditTurunanVue from './DialogRincianEditTurunan.vue'
    export default {
        ...
        computed:{
            ...
            cIsi(){
                return this.$store.state.kuesioner.blok_10.isi.map((e, i) => {
                    return e.reduce((p,c) => {
                        p[c.nama] = c.isi
                        p.index = i
                        return p
        ...
        methods:{
            editRow(e){
                ...
                this.selectedIsi = this.$store.state.kuesioner.blok_10.isi[e]
                this.editDialog = true
                // console.log('editrow', this.editDialog, this.$store.state.kuesioner.blok_10.isi[e])
            },

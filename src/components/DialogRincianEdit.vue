<template>
    <v-dialog 
        ...
        <v-dialog v-model="hapusPrevent" v-if="!noDelete">
            <v-card>
                <v-card-title>Apakah kamu yakin untuk menghapusnya?</v-card-title>
                <v-card-actions>
                    <v-btn color="primary" dark @click="deleteIsi">
                        Ya
                    </v-btn>
                    <v-btn color="danger" dark @click="(hapusPrevent = false)">
                        Batal
                    </v-btn>
                </v-card-actions>
            </v-card>
...
import RincianVue from './kuesioner/Rincian.vue'
    export default {
        props: ["rincian", "showDialog", "isi", "noDelete"],
        components:{
